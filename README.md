...
### Известный дефект: сводные и сырые колонки расходятся в 177 кейсах

В 177 кейсах из 660 поле `<metric>_effect_relative` не совпадает с эффектом,
пересчитанным из сырых колонок `revenue` и `n_users`, а строки по сегментам
не складываются в строку `all`. Поголовно затронуты `simpson_paradox`,
`harking`, `heterogeneity`, `srm` и `underpowered`; в отдельных кейсах
расходится даже знак эффекта. Отдельно: `simpson_paradox` в текущей версии
**не воспроизведён** — доли трафика в обеих группах одинаковы, а без сдвига
микса парадокса Симпсона не бывает.

На опубликованные цифры это не влияет: агент читает `effect_relative`,
и эталон построен на том же поле, поэтому 42.7%, «20 из 33» и разложение
362/10 внутренне согласованы.

**До починки не пересчитывайте эффекты из сырых колонок** — берите
`effect_relative` и `p_value` как есть. Дефект чинится, подробности
и план в [BACKLOG.md](BACKLOG.md).
