...
механизмами — и **42.6%** на основном корпусе из 660 кейсов с 33 механизмами.
...
|---|---|---|---|
| `ship` — катить | 361 | **10** | **351** |
| `investigate` — разбираться | 161 | 135 | 26 |
| `do_not_ship` — не катить | 138 | **136** | **2** |

Правило разрешило выкатку 361 раз и было право десять раз — причём эти
десять и есть все кейсы корпуса, где ловушки нет вообще. Когда правило
отказывает, оно почти всегда право: 136 из 138.
...
виде ошибки. Из 351 ложного разрешения 260 требовали разбирательства,
а 91 — прямого запрета.
...
Проверить себя на тех же кейсах можно в
**[тренажёре](https://yzotop.github.io/ab-factory-demo/trainer.html)**: пятнадцать
экспериментов, каждый решается по одной таблице без статистики, после ответа
видно, куда стоило смотреть. Там же рядом —
[обозреватель корпуса](https://yzotop.github.io/ab-factory-demo/corpus.html)
со всеми 660 кейсами.

## Что корпус говорит о себе

Три вещи, которые нужно знать **до** того, как на корпусе что-то измерять.
Все три обнаружены на нём же: две меняют интерпретацию результатов, третья —
дефект данных, который чинится.
...
Седьмой механизм, `simpson_paradox`, называл себя в `notes` во всех 26 кейсах
и в перепись выше не попадал — формулировка «aggregate uplift masks negative
segment effects (mix shift)» выдавала разбор готовым. Сейчас `notes` там
описывают только причину сдвига трафика, а вывод из неё читатель делает сам.
...
### Данные согласованы, и это проверяется командой

Сводные и сырые колонки в корпусе связаны определениями, а не совпадают
случайно. Проверяются три тождества:

- строки по сегментам складываются в строку `all` — по выручке и по
  пользователям;
- `<metric>_effect_relative` равен эффекту, пересчитанному из сырых колонок
  на пользователя;
- `revenue` равен `shows × cpm / 1000`.

```bash
cd 42_workflows/ab_factory
python3 selfcheck.py --invariants --root ../../40_ab_factory/vk-style/cases_mvp_v2
```

Сейчас проходят все 660 кейсов и все 100 в учебном наборе. **Считайте
эффекты из сырых колонок, если вам так удобнее** — они сойдутся.

Так было не всегда. В опубликованной ранее версии тождества нарушались
в 638 кейсах из 660: генератор разыгрывал выручку, показы и CPM независимо,
а эффект применял к суммарной выручке, тогда как размер группы дрожал сам
по себе. Проверки не существовало, поэтому дефект никто не видел — self-check
сверял только вердикт агента с эталоном. Проверка написана **до** починки,
чтобы масштаб был виден числом; разбор — в [BACKLOG.md](BACKLOG.md).

## Что внутри

**Три корпуса.** По одному вшитому механизму на кейс:
...
- `40_ab_factory/vk-style/cases_trainer/` — **40 кейсов**, дополнение для
  тренажёра. В основной корпус не входят и в его цифры не попадают: это
  чистые выкатки, добавленные, чтобы у тренажёра были все три ответа
  примерно поровну.
...
# 660 кейсов, 33 механизма → 42.6% (281 из 660)
python3 selfcheck.py --root ../../40_ab_factory/vk-style/cases_mvp_v2
```

Пересобрать корпуса с нуля (детерминированно, `seed=42`):

```bash
./43_generation/ab_factory/run.sh --n 660 --out 40_ab_factory/vk-style/cases_mvp_v2
```

```bash
./43_generation/ab_factory/run.sh --profile auto
```
