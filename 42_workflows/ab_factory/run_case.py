...
def make_run_id() -> str:
    # 32 бита случайности, а не 16. Метка времени идёт с точностью до
    # секунды, а весь корпус прогоняется быстрее: 100 кейсов за 0.13 с,
    # 660 — за 0.8 с. То есть все прогоны партии попадают в одну-две
    # секунды, и уникальность держится только на случайной части.
    # На 16 битах это задача о днях рождения: ~7% шанс совпадения
    # на 100 кейсах и ~96% на 660. При совпадении второй прогон
    # переписывает артефакты первого, и кейс молча исчезает из индекса
    # (наблюдалось: 100 папок, 99 кейсов, пропал case_059).
    # На 32 битах те же 660 дают ~0.005%.
    # Метка времени остаётся первой: build_corpus_index сравнивает
    # имена папок лексикографически, чтобы взять последний прогон.
    ts = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    rnd = f"{random.randint(0, 0xFFFFFFFF):08x}"
    return f"{ts}_{rnd}"
